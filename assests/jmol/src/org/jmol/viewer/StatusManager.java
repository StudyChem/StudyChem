/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2010-09-22 17:32:26 -0500 (Wed, 22 Sep 2010) $
 * $Revision: 14363 $
 *
...
////////////////////Jmol status //////////////
  ...
  synchronized private void setStatusChanged(String statusName,
    ...
    Vector statusRecordSet = (isReplace ? null : (Vector)messageQueue.get(statusName));
    if (statusRecordSet == null)
      messageQueue.put(statusName, statusRecordSet = new Vector());
    if (statusRecordSet.size() == MAXIMUM_QUEUE_LENGTH)
      statusRecordSet.removeElementAt(0);    
    statusRecordSet.addElement(msgRecord);
  }
  
  synchronized Vector getStatusChanged(String newStatusList) {
    /*
    ...
    if (getList) {
      ...
      while (e.hasMoreElements())
        msgList.addElement(messageQueue.remove(e.nextElement()));
    } else {
      messageQueue.clear();
    }
    statusPtr = 0;
